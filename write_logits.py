...
def launch_graph_and_train_with_summaries(lr=1e-4,kp=0.2,batch_size=50,num_epochs=20,sess=None):
    ...
    # FC2 Layer
    y_conv = nn_layer(h_fc1_drop,1024,10,'fc2',act=tf.identity,operation=tf.matmul) 
    ...
    for i in range(num_epochs*iters_per_epoch):
        ...
        # Log test stats
        if i%50 == 0:
            summary, val_accuracy = sess.run([merged,accuracy],feed_dict={x:mnist.validation.images, y_: mnist.validation.labels, keep_prob: 1.0}) 
    ...
    val_acc = accuracy_in_batches(mnist.validation, accuracy, x, y_, keep_prob, batch_size=batch_size)
...
def fine_tune():
    ...
    for lr in lrs:
        for kp in keep_probs:
            ...
            _, _, _, _, _, val_acc = launch_graph_and_train(lr=lr,kp=kp,batch_size=batch_size,num_epochs=num_epochs,sess=sess)
...
def write_best_logits(sess=None, lr=0.000517947, kp=0.2):
    ...
    x, y_, y_conv, keep_prob, accuracy_tf, _ = launch_graph_and_train_with_summaries(lr=lr,kp=kp,batch_size=batch_size,num_epochs=num_epochs,sess=sess)
